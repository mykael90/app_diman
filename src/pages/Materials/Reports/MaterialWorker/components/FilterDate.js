...
function FilterDate({ getData }) {
  ...
  const handleStore = async (values, resetForm) => {
    ...
    formattedValues.id = formattedValues.idMaterial
      .map((obj, index) => `id[${index + 1}]=${obj.materialId}`)
      .join('&');
      ...
      >
        {({
          ...
          <Form noValidate autoComplete="off" onSubmit={handleSubmit}>
            ...
            >
              <span className="fs-6">
                LISTA DE MATERIAIS A SEREM PESQUISADOS
              </span>
            </Row>
            <FieldArray name="idMaterial">
              {(fieldArrayProps) => {
                ...
                return (
                  <>
                    ...
                    <Row style={{ background: body2Color }}>
                      {values.idMaterial?.length > 0 &&
                        values.idMaterial?.map((item, index) => (
                          <>
            ...
            <Row className="pt-4">
              <Col xs="auto">
                {touched.idMaterial && typeof errors.idMaterial === 'string' ? (
                  <Badge bg="danger">{errors.idMaterial}</Badge>
