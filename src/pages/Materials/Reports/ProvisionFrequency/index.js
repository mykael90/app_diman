...
export default function Index() {
    ...
    try {
      ...
      const groupData = response.data
        .filter(
          (value, index, arr) =>
            arr.findIndex(
              (item) => item.materialIdIn === value.materialIdIn
            ) === index
        )
        .map((value) => ({
          materialIdIn: value.materialIdIn,
          name: value.name,
          unit: value.unit,
        })); // RETORNA OS DIFERENTES MATERIAIS

      groupData.forEach((mat) => {
        mat.input = response.data.filter(
          (value) => mat.materialIdIn === value.materialIdIn
        );
      });

      groupData.forEach((mat) => {
        mat.totalProvision = mat.input
          .reduce((ac, value) => {
            ac += Number(value.sumQuantityIn);
            return ac;
          }, 0)
          .toFixed(2);
        mat.totalPrice = mat.input
          .reduce((ac, value) => {
            ac += Number(value.totalPrice);
            return ac;
          }, 0)
          .toFixed(2);
        mat.totalFrequency = mat.input
          .reduce((ac, value) => {
            ac += Number(value.inputFrequency);
            return ac;
          }, 0)
          .toFixed(2);
        mat.PF = (mat.totalProvision / mat.totalFrequency).toFixed(2);
      });
  ...
  const columns = React.useMemo(
    () => [
      ...
      {
        Header: 'Provisão Total',
        accessor: 'totalProvision',
        width: 100,
        disableResizing: true,
        Cell: ({ value }) => (
    ...
    ({ row }) => (
      <TableNestedrow
        ...
        columns={[
          ...
          {
            Header: 'Provisão',
            accessor: 'sumQuantityIn',
            width: 100,
            disableResizing: true,
          },
          {
            Header: 'Frequência',
            accessor: 'inputFrequency',
            width: 100,
            disableResizing: true,
          },
          {
            Header: 'Preço Unit.',
            accessor: 'maxPriceValueInBr',
            width: 100,
            disableResizing: true,
          },
          {
    ...
    <>
      ...
      <Container>
        <Row className="text-center py-3">
          <Card.Title>
            Provisões e frequências de abastecimento de materiais
          </Card.Title>
          <Card.Text>
            Abastecimentos (desconsiderados os retornos) e frequências (nº de
            vezes que o material constou em entradas para abastecimento) para o
            período informado. <br /> Data de início do sistema em 14/10/2022.
          </Card.Text>
        </Row>
